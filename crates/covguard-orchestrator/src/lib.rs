...
>(
    ...
    let domain_scope = request.scope;
    ...
    let markdown = render_markdown_with_limit(&domain_report, output.max_markdown_lines);
    let annotations = render_annotations_with_limit(&domain_report, output.max_annotations);
    let sarif = render_sarif_with_limit(&domain_report, output.max_sarif_results);
...
// truncate_findings moved to `covguard_output` for shared API semantics.
...
fn build_reasons(output: &EvalOutput) -> Vec<String> {
    reporting_build_reasons(output)
}
...
// Rendering convenience functions are re-exported from `covguard_output`
// to keep output formatting responsibilities centralized.

// ============================================================================
// Ignore Directive Detection
// ============================================================================
//
// Detection is delegated to `covguard_directives::detect_ignored_lines` and
// re-exported from this module for backward-compatible API access.
...
"#;
    ...
    fn test_truncate_findings_no_truncation_when_under_limit() {
        ...
        let (truncated, trunc) = covguard_output::truncate_findings(findings.clone(), Some(5));
