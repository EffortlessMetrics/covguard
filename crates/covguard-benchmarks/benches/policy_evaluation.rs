...
fn generate_coverage_map(
    ...
    for file_idx in 0..num_files {
        ...
        for line_idx in 1..=lines_per_file {
            ...
            file_coverage.insert(line_idx as u32, hits as u32);
        }
    ...
    for (path, ranges) in changed_ranges {
        ...
        for range in ranges {
            for line in range.clone() {
                match file_coverage {
                    Some(coverage_map) => match coverage_map.get(&line) {
...
fn bench_policy_evaluation(c: &mut Criterion) {
    ...
    for (num_files, lines_per_file) in [(10, 100), (50, 200), (100, 500), (500, 200)] {
        ...
        let total_lines: usize = changed_ranges
            .values()
        ...
        group.bench_with_input(
            ...
            |b, (changed_ranges, coverage)| {
                b.iter(|| evaluate_policy(changed_ranges, coverage, 80.0));
            },
        );
    }
...
fn bench_ignored_lines(c: &mut Criterion) {
    ...
    group.bench_function("batch_check_ignored", |b| {
        b.iter(|| {
            let mut ignored_count = 0;
            for ignored_set in ignored.values() {
