...
use covguard_adapters_diff::{parse_patch, parse_patch_with_meta, normalize_path};
use std::fs;
...
fn test_fixture_multiple_files() {
    ...
    assert_eq!(calc_ranges, &vec![1..=11], "Expected lines 1-11 in src/calculator.rs");
    
    // validator.rs: lines 1-7 added
    let val_ranges = result.changed_ranges.get("src/validator.rs").unwrap();
    assert_eq!(val_ranges, &vec![1..=7], "Expected lines 1-7 in src/validator.rs");
}
...
fn test_fixture_renamed_file() {
    ...
    assert!(result.changed_ranges.contains_key("src/utils.rs"),
        "Expected src/utils.rs (new name) in changed_ranges");
    assert!(!result.changed_ranges.contains_key("src/old_utils.rs"),
...
fn test_fixture_binary_file() {
    ...
    assert!(result.changed_ranges.contains_key("src/config.rs"),
        "Expected src/config.rs in changed_ranges");
    
    // Binary file should NOT be in changed_ranges
    assert!(!result.changed_ranges.contains_key("assets/logo.png"),
        "Binary file should not be in changed_ranges");
    
    let config_ranges = result.changed_ranges.get("src/config.rs").unwrap();
    assert_eq!(config_ranges, &vec![3..=4], "Expected lines 3-4 in src/config.rs");
}
...
fn test_fixture_with_ignore_directive() {
    ...
    assert_eq!(ranges, &vec![1..=3], "Expected lines 1-3 (ignore directive is not parsed by diff parser)");
}
...
+line two\r\n";
    ...
    println!("Result: {:?}", result);
    assert_eq!(result.get("src/lib.rs"), Some(&vec![1..=2]),
...
"#;
    ...
    assert_eq!(ranges, &vec![1000001..=1000002], 
...
"#;
    ...
    println!("Result: {:?}", result);
    assert!(result.is_empty(), "Deleted file should have no added ranges");
}
...
"#;
    ...
    assert!(!result.contains_key("old_name.rs"), "Should not use old name");
}
...
"#;
    ...
    println!("Result: {:?}", result);
    assert!(result.contains_key("new_dir/file.rs"), "Should use new path");
}
...
fn test_fuzz_crlf_endings() {
    let content = fs::read_to_string(fixture_path("fuzz/corpus/fuzz_diff_parser/crlf_endings.patch"))
...
fn test_fuzz_large_line_numbers() {
    let content = fs::read_to_string(fixture_path("fuzz/corpus/fuzz_diff_parser/large_line_numbers.patch"))
...
fn test_fuzz_empty_hunk() {
    let content = fs::read_to_string(fixture_path("fuzz/corpus/fuzz_diff_parser/empty_hunk.patch"))
...
fn test_all_fixtures_summary() {
    ...
    let fixtures = [
        ...
        (fixture_path("fixtures/diff/with_ignore_directive.patch"), true, 1),
    ];
    ...
    for (path, expect_content, expected_files) in fixtures {
        ...
        let result = parse_patch_with_meta(&content).expect(&format!("Failed to parse {}", path));
        
        let file_count = result.changed_ranges.len();
        let binary_count = result.binary_files.len();
        let total_lines: usize = result.changed_ranges.values()
        ...
        if expect_content && file_count != expected_files {
            println!("  WARNING: Expected {} files, got {}", expected_files, file_count);
        }
    }
